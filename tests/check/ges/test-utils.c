...
static void
week_notify (DestroyedObjectStruct * destroyed, GObject ** object)
{
...
check_destroyed (GObject * object_to_unref, GObject * first_object, ...)
{
  ...
  g_object_weak_ref (object_to_unref, (GWeakNotify) week_notify, destroyed);
  ...
  if (first_object) {
    ...
    while (object) {
      ...
      g_object_weak_ref (object, (GWeakNotify) week_notify, destroyed);
      objs = g_list_prepend (objs, destroyed);
