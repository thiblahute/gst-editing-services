...
/* GST_START_TEST (simple_smart_adder_pipeline_test)
 * {
 *   GESTrack *track = GES_TRACK (ges_audio_track_new ());
 *   GstElement *pipeline, *audiotestsrc, *audiotestsrc1, *smart_adder, *fakesink;
 *
 *   pipeline = gst_pipeline_new ("smart-adder-pipeline");
 *   audiotestsrc = gst_parse_bin_from_description ("audiotestsrc wave=triangle", TRUE, NULL);
 *   audiotestsrc1 = gst_parse_bin_from_description ("audiotestsrc wave=blue-noise", TRUE, NULL);
 *   smart_adder = ges_smart_adder_new (track);
 *   fakesink = gst_element_factory_make ("fakesink", NULL);
 *
 *   gst_bin_add_many (GST_BIN (pipeline), audiotestsrc, audiotestsrc1,
 *       smart_adder, fakesink, NULL);
 *   fail_unless (gst_element_link (audiotestsrc, smart_adder));
 *   fail_unless (gst_element_link (audiotestsrc1, smart_adder));
 *   fail_unless (gst_element_link (smart_adder, fakesink));
 *
...
GST_START_TEST (simple_audio_mixed_with_pipeline)
{
  ...
  tmpclip = ges_timeline_layer_add_asset (layer, asset, 0, 0,
      1 * GST_SECOND, 1, GES_TRACK_TYPE_AUDIO);
  ...
  tmpclip = ges_timeline_layer_add_asset (layer1, asset, 0, 0, 2 * GST_SECOND,
      1, GES_TRACK_TYPE_AUDIO);
...
/*   tcase_add_test (tc_chain, simple_smart_adder_pipeline_test);
 */
