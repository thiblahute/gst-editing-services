...
struct _GESTrackElementPrivate
{
  ...
  GstElement *nleobject;        /* The GnlObject */
...
ensure_nle_object (GESTrackElement * object)
{
  ...
  if (object->priv->nleobject == NULL) {
    ...
    if (G_UNLIKELY (nleobject == NULL)) {
      GST_ERROR
          ("'create_nle_object' implementation returned TRUE but no GnlObject is available");
      goto done;
    }

    GST_DEBUG_OBJECT (object, "Got a valid GnlObject, now filling it in");

    object->priv->nleobject = nleobject;
    g_object_set_qdata (G_OBJECT (nleobject), GNL_OBJECT_TRACK_ELEMENT_QUARK,
        object);
