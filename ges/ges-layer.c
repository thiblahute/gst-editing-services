...
ges_layer_init (GESLayer * self)
{
  ...
  self->min_nle_priority = MIN_GNL_PRIO;
  self->max_nle_priority = LAYER_HEIGHT + MIN_GNL_PRIO;
...
ges_layer_set_priority (GESLayer * layer, guint priority)
{
  ...
  if (priority != layer->priv->priority) {
    ...
    layer->min_nle_priority = (priority * LAYER_HEIGHT) + MIN_GNL_PRIO;
    layer->max_nle_priority = ((priority + 1) * LAYER_HEIGHT) + MIN_GNL_PRIO;
