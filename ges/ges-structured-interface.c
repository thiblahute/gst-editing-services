...
_check_field (GQuark field_id, const GValue * value, FieldsError * fields_error)
{
  ...
  fields_error->invalids =
      g_list_append (fields_error->invalids, (gpointer) field);
...
gboolean
_ges_add_add_clip_from_struct (GESTimeline * timeline, GstStructure * structure,
    GError ** error)
{
  GESAsset *asset;
  GESLayer *layer;
