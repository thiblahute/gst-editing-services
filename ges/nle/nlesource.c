...
G_DEFINE_TYPE_WITH_CODE (GnlSource, nle_source, GNL_TYPE_OBJECT, _do_init);
...
static gboolean nle_source_prepare (GnlObject * object);
...
static gboolean
nle_source_control_element_func (GnlSource * source, GstElement * element);

static void
nle_source_class_init (GnlSourceClass * klass)
{
...
static void
nle_source_init (GnlSource * source)
{
    ...
    GnlSource * source)
{
  GstCaps *srccaps;
  GnlSourcePrivate *priv = source->priv;
  GnlObject *nleobject = (GnlObject *) source;
    ...
    GnlSource * source)
{
  ...
  if (pad == priv->ghostedpad) {
    ...
    nle_object_ghost_pad_set_target (GNL_OBJECT (source), nleobject->srcpad,
        NULL);
...
nle_source_control_element_func (GnlSource * source, GstElement * element)
{
  ...
  if (get_valid_src_pad (source, source->element, &pad)) {
    priv->staticpad = pad;
    nle_object_ghost_pad_set_target (GNL_OBJECT (source),
...
{
  GnlSource *source = (GnlSource *) bin;
  GnlObject *nleobject = (GnlObject *) element;
  GnlSourcePrivate *priv = source->priv;
  gboolean pret;
...
static gboolean
nle_source_prepare (GnlObject * object)
{
  GstPad *pad;
