...
G_DEFINE_TYPE_WITH_CODE (GnlOperation, nle_operation, GNL_TYPE_OBJECT,
    _do_init);
...
static gboolean nle_operation_prepare (GnlObject * object);
static gboolean nle_operation_cleanup (GnlObject * object);
...
static void
nle_operation_class_init (GnlOperationClass * klass)
{
  GObjectClass *gobject_class = (GObjectClass *) klass;
  GstBinClass *gstbin_class = (GstBinClass *) klass;

  GstElementClass *gstelement_class = (GstElementClass *) klass;
  GnlObjectClass *nleobject_class = (GnlObjectClass *) klass;
...
nle_operation_dispose (GObject * object)
{
  ...
  nle_object_ghost_pad_set_target (GNL_OBJECT (object),
...
static void
nle_operation_reset (GnlOperation * operation)
{
...
static void
nle_operation_init (GnlOperation * operation)
{
  ...
  if (operation->element) {
    ...
    if (!element_is_valid_filter (element, &isdynamic)) {
      ...
      if ((res = GST_BIN_CLASS (parent_class)->add_element (bin, element))) {
        ...
        nle_object_ghost_pad_set_target (GNL_OBJECT (operation),
...
static void
nle_operation_set_sinks (GnlOperation * operation, guint sinks)
{
...
add_sink_pad (GnlOperation * operation)
{
  ...
  if (!operation->dynamicsinks) {
    ...
    if (ret) {
      gpad = nle_object_ghost_pad ((GnlObject *) operation, GST_PAD_NAME (ret),
          ret);
...
remove_sink_pad (GnlOperation * operation, GstPad * sinkpad)
{
  ...
  if (sinkpad) {
    ...
    if (target) {
      ...
      nle_object_ghost_pad_set_target ((GnlObject *) operation, sinkpad, NULL);
      if (operation->dynamicsinks)
        gst_element_release_request_pad (operation->element, target);
      gst_object_unref (target);
    }
    operation->sinks = g_list_remove (operation->sinks, sinkpad);
    nle_object_remove_ghost_pad ((GnlObject *) operation, sinkpad);
    operation->realsinks--;
  }
...
static gboolean
nle_operation_prepare (GnlObject * object)
{
  /* Prepare the pads */
...
static gboolean
nle_operation_cleanup (GnlObject * object)
{
  GnlOperation *oper = (GnlOperation *) object;
...
void
nle_operation_hard_cleanup (GnlOperation * operation)
{
  gboolean done = FALSE;
  ...
  nle_object_cleanup (GNL_OBJECT (operation));
}
...
void
nle_operation_signal_input_priority_changed (GnlOperation * operation,
...
void
nle_operation_update_base_time (GnlOperation * operation,
    GstClockTime timestamp)
{
  if (!nle_object_to_media_time (GNL_OBJECT (operation),
