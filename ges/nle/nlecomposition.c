...
G_DEFINE_TYPE_WITH_CODE (GnlComposition, nle_composition, GNL_TYPE_OBJECT,
    _do_init);
...
static GParamSpec *nleobject_properties[GNLOBJECT_PROP_LAST];
...
static void nle_composition_reset (GnlComposition * comp);
...
static inline void nle_composition_reset_target_pad (GnlComposition * comp);
...
static gboolean update_pipeline (GnlComposition * comp,
    ...
    GnlUpdateStackReason update_stack_reason);
static gboolean nle_composition_commit_func (GnlObject * object,
    gboolean recurse);
...
static gboolean
_nle_composition_add_object (GnlComposition * comp, GnlObject * object);
static gboolean
_nle_composition_remove_object (GnlComposition * comp, GnlObject * object);
...
static void
nle_composition_class_init (GnlCompositionClass * klass)
{
  GObjectClass *gobject_class;
  GstElementClass *gstelement_class;
  GstBinClass *gstbin_class;
  GnlObjectClass *nleobject_class;

  gobject_class = (GObjectClass *) klass;
  gstelement_class = (GstElementClass *) klass;
  gstbin_class = (GstBinClass *) klass;
  nleobject_class = (GnlObjectClass *) klass;
   ...
   * fast notifies */
  nleobject_properties[GNLOBJECT_PROP_START] =
      g_object_class_find_property (gobject_class, "start");
  nleobject_properties[GNLOBJECT_PROP_STOP] =
      g_object_class_find_property (gobject_class, "stop");
  nleobject_properties[GNLOBJECT_PROP_DURATION] =
...
static void
nle_composition_init (GnlComposition * comp)
{
  ...
  priv->nle_event_pad_func = GST_PAD_EVENTFUNC (GNL_OBJECT_SRC (comp));
...
_remove_child (GValue * item, GValue * ret G_GNUC_UNUSED, GstBin * bin)
{
  ...
  if (GNL_IS_OPERATION (child))
    nle_operation_hard_cleanup (GNL_OPERATION (child));
...
static void
nle_composition_reset (GnlComposition * comp)
{
  GnlCompositionPrivate *priv = comp->priv;
...
static gboolean
nle_composition_commit_func (GnlObject * object, gboolean recurse)
{
...
update_base_time (GNode * node, GstClockTime * timestamp)
{
  if (GNL_IS_OPERATION (node->data))
    nle_operation_update_base_time (GNL_OPERATION (node->data), *timestamp);
    ...
    GstEvent * event)
{
  ...
  if (res) {
    ...
    res = priv->nle_event_pad_func (GNL_OBJECT (comp)->srcpad, parent, event);
...
static inline void
nle_composition_reset_target_pad (GnlComposition * comp)
{
  GnlCompositionPrivate *priv = comp->priv;
  ...
  nle_object_ghost_pad_set_target (GNL_OBJECT (comp),
 ...
 */
static void
nle_composition_ghost_pad_set_target (GnlComposition * comp, GstPad * target)
{
  GstPad *ptarget;
  GnlCompositionPrivate *priv = comp->priv;
  ...
  nle_object_ghost_pad_set_target ((GnlObject *) comp,
      ...
      if (object->stop >= timestamp) {
        if ((object->start < timestamp) &&
          ...
          if (GNL_IS_OPERATION (object))
            nle_operation_update_base_time (GNL_OPERATION (object), timestamp);
        }
      } else {
  ...
  if (G_LIKELY (timestamp < GNL_OBJECT_STOP (comp)))
    for (tmp = comp->priv->expandables; tmp; tmp = tmp->next) {
      ...
      if (GNL_IS_OPERATION (tmp->data))
        nle_operation_update_base_time (GNL_OPERATION (tmp->data), timestamp);
    }
  ...
  if (obj->stop != cobj->stop) {
    ...
    if (priv->expandables) {
      ...
      for (tmp = priv->expandables; tmp; tmp = tmp->next) {
        ...
        nle_object_commit (GNL_OBJECT (tmp->data), FALSE);
      }
    }
    ...
    GstEvent * toplevel_seek)
{
  ...
  if (newparent) {
    ...
    nle_operation_signal_input_priority_changed ((GnlOperation *)
...
_nle_composition_add_object (GnlComposition * comp, GnlObject * object)
{
  ...
  nle_object_set_caps (object, GNL_OBJECT (comp)->caps);
  nle_object_set_commit_needed (GNL_OBJECT (comp));
  ...
  if (GNL_OBJECT_IS_EXPANDABLE (object)) {
    ...
    nle_object_commit (GNL_OBJECT (object), FALSE);
  }
  ...
  if (G_UNLIKELY (!gst_caps_is_any (((GnlObject *) comp)->caps)))
    nle_object_set_caps ((GnlObject *) object, ((GnlObject *) comp)->caps);
...
_nle_composition_remove_object (GnlComposition * comp, GnlObject * object)
{
  ...
  nle_object_reset (GNL_OBJECT (object));
