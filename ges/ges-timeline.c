    ...
    GESTrackElement * trackelement)
{
  ...
  if (G_LIKELY (iters->iter_by_layer)) {
    if (!g_sequence_iter_is_end (iters->iter_by_layer))
      g_sequence_remove (iters->iter_by_layer);
    else
      GST_WARNING_OBJECT (timeline, "Why is that happening?!");
  } else {
  ...
  if (G_UNLIKELY (layer == NULL)) {
    ...
    if (layer != iters->layer) {
      ...
      GST_DEBUG_OBJECT (child, "Moved from layer %" GST_PTR_FORMAT
          ...
          ges_timeline_layer_get_priority (layer), iters->layer,
          ges_timeline_layer_get_priority (iters->layer));
