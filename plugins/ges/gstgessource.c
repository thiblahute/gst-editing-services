 ...
 * @short_description: A source element that wraps a GESTimeline.
 ...
 * uri scheme.
 *
 * When used with playbin, you can set its uri to ges:// and connect to
 * the source-setup to setup the timeline property of that GstGESSource
 * at the right time.
 **/
...
gst_ges_source_set_timeline (GstGesSource * self, GESTimeline * timeline)
{
  ...
  for (tmp = self->timeline->tracks; tmp; tmp = tmp->next) {
    ...
    else if (GES_TRACK (tmp->data)->type == GES_TRACK_TYPE_VIDEO)
    ...
    GError ** error)
{
  ...
  if (gst_uri_is_valid (location)) {
    GESTimeline *timeline = ges_timeline_new_from_uri (location, error);

    if (!timeline) {
      GST_ERROR_OBJECT (self, "Could not create timeline");
      return FALSE;
    }

    gst_ges_source_set_timeline (self, timeline);
  }
